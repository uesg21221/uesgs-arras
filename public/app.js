...
(async function (util, global, settings, Canvas, color, gameDraw, socketStuff) {
    ...
    const drawEntity = (baseColor, x, y, instance, ratio, alpha = 1, scale = 1, lineWidthMult = 1, rot = 0, turretsObeyRot = false, assignedContext = false, turretInfo = false, render = instance.render) => {
        ...
        for (let i = 0; i < source.turrets.length; i++) {
            ...
            t.lerpedFacing == undefined ? (t.lerpedFacing = t.facing) : (t.lerpedFacing = util.lerpAngle(t.lerpedFacing, t.facing, 0.1, true));
        ...
        context.shadowColor = m.glow.color != null ? gameDraw.modifyColor(m.glow.color) : gameDraw.mixColors(gameDraw.modifyColor(instance.color), render.status.getColor(), render.status.getBlend());
    ...
    function drawUpgradeTree(spacing, alcoveSize) {
        ...
        global.fixedScrollX = Math.max(dividedWidth - padding, Math.min(tankTree.width * treeFactor + padding - dividedWidth, global.fixedScrollX + global.scrollVelocityX));
        global.fixedScrollY = Math.max(dividedHeight - padding, Math.min(tankTree.height * treeFactor + padding - dividedHeight, global.fixedScrollY + global.scrollVelocityY));
    ...
    function drawMessages(spacing, alcoveSize) {
        ...
        for (let i = global.messages.length - 1; i >= 0; i--) {
            ...
            if (msg.status > 1) {
                msg.status -= 0.05;
                msg.alpha += 0.05;
            } else if (i === 0 && (global.messages.length > 5 || Date.now() - msg.time > 0)) {
                msg.status -= 0.05;
                msg.alpha -= 0.05;
                // Remove
                if (msg.alpha <= 0) {
    ...
    function drawMobileJoysticks() {
        // Draw the joysticks.
        let radius = Math.min(global.screenWidth * 0.8, global.screenHeight * 0.16);
        ctx.globalAlpha = 0.3;
        ...
        ctx.fillStyle = '#ffffff';
        ctx.beginPath();
        ctx.arc(canvas.movementTouchPos.x + (global.screenWidth * 1) / 6, canvas.movementTouchPos.y + (global.screenHeight * 2) / 3, radius / 2.5, 0, 2 * Math.PI);
        ctx.arc(canvas.controlTouchPos.x + (global.screenWidth * 5) / 6, canvas.controlTouchPos.y + (global.screenHeight * 2) / 3, radius / 2.5, 0, 2 * Math.PI);
        ctx.fill();
        // crosshair
        const crosshairpos = {
    ...
    let getKills = () => {
        ...
        return (
            (destruction === 0 ? '🌼' : destruction < 4 ? '🎯' : destruction < 8 ? '💥' : destruction < 15 ? '💢' : destruction < 25 ? '🔥' : destruction < 50 ? '💣' : destruction < 75 ? '👺' : destruction < 100 ? '🌶️' : '💯') +
            ' ' +
            (!killCountTexts.length ? 'A true pacifist' : killCountTexts.length == 1 ? killCountTexts.join(' and ') : killCountTexts.slice(0, -1).join(', ') + ' and ' + killCountTexts[killCountTexts.length - 1])
        );
    };
    let getDeath = () => {
        let txt = '';
    ...
    const gameDrawDead = () => {
        ...
        drawText(
            global.cannotRespawn ? (global.respawnTimeout ? '(' + global.respawnTimeout + ' Secon' + `${global.respawnTimeout <= 1 ? 'd' : 'ds'} ` + 'left to respawn)' : '(You cannot respawn)') : global.mobile ? '(Tap to respawn)' : '(Press enter to respawn)',
            x,
            y + 189,
            16,
            color.guiwhite,
