...
module.exports = class Siege extends Gamemode {
    constructor () {
        super();
    ...
    stop () {
        c.BOSS_SPAWN_COOLDOWN /= 2 ** 24;
        if (Class.basic.UPGRADES_TIER_2.includes("healer")) {
            Class.basic.UPGRADES_TIER_2.splice(Class.basic.UPGRADES_TIER_2.indexOf("healer"), 1);
        }
    }
    ...
    afterWaveDefeat (siegeInstance = this) {
        siegeInstance.waveId++;
        if (siegeInstance.waves[siegeInstance.waveId]) {
            siegeInstance.spawnWave(siegeInstance.waveId);

        //if there is no next wave then simply let the players win
        } else {
            siegeInstance.playerWin();
        }
    }
    ...
    spawnEnemyWrapper(loc, type) {
        ...
        enemy.on('dead', () => {
            ...
            if (!--this.remainingEnemies) {
                ...
                setSyncedTimeout(this.afterWaveDefeat, 5 * 30, this);
            }
        });
        return enemy;
    }
