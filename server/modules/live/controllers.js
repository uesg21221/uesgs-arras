...
think() {
    if (!this.timer--) {
        ...
        this.pathAngle -= ((this.body.velocity.length / 90) * Math.PI) / c.runSpeed * 5;
    }
    return {
        goal: this.goal,
...
think(input) {
    ...
    this.velocityMagnitude = Math.min(0.1, this.velocityMagnitude + 0.01 / c.runSpeed)
}
}
...
constructor(body) {
    super(body);
    let handler = () => {
        this.sizeFactor = this.body.size / this.body.master.size;
    };
