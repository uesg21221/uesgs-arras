...
normal = new Tile({
    color: "white",
    data: { allowMazeWallSpawn: true, foodSpawnCooldown: 0, foodCount: 0 },
    ...
    tick: tile => {
        if (++tile.data.foodSpawnCooldown > c.FOOD_SPAWN_COOLDOWN) {
            ...
            if (data.foodCount < c.FOOD_CAP && Math.random() < c.FOOD_SPAWN_CHANCE) {
...
nest = new Tile({
    ...
    tick: tile => {
        ...
        if (++tile.data.foodSpawnCooldown > c.FOOD_SPAWN_COOLDOWN_NEST) {
            tile.data.foodSpawnCooldown = 0;
            if (data.foodCount < c.FOOD_CAP_NEST && Math.random() < c.FOOD_SPAWN_CHANCE_NEST) {
