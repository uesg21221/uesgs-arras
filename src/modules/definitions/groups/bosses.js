        ...
        GUNS: [
            {
                ...
                PROPERTIES: {
                    SHOOT_SETTINGS: combineStats([g.basic, g.skimmer, {reload: 0.5}, g.lowPower, {recoil: 1.35}, {speed: 1.3, maxSpeed: 1.3}, {speed: 1.3, maxSpeed: 1.3}, {reload: 0.15, recoil: 1, range: 0.1}]),
                    TYPE: [
                        'bullet',
                        {
    ...
    GUNS: [
        ...
        ...Array(10)
            ...
            .map((_, i) => ({
                ...
                PROPERTIES: {
                    SHOOT_SETTINGS: combineStats([g.basic, g.twin, g.gunner, g.cyclone, {reload: 3}]),
                    TYPE: 'bullet',
                    AUTOFIRE: true,
                },
            })),
...
Class.trplnrBoss = {
    ...
    ...trplnrBossDecor,
    UPGRADE_TOOLTIP:
        '"Heck, even The Guardians are afraid of him, \n' +
    ...
    ON: [
        {
            ...
            handler: ({body, gun}) => {
                ...
                const messages = ['Attack my little swarms!', 'Deploying, Attack swarms', 'You really think you can defeat me? Heres a little challenge for you.', 'This thing is really gonna annoy you HAHA!', "I don't know what to say uhhh, die i guess."];
    ...
    ON: [
        {
            ...
            handler: ({body, masterStore, gun}) => {
                ...
                if (masterStore.shotsFired > 5) {
                    ...
                    const messages = ["I'm a little tired right now", 'Ouch my leg!', 'i sleep', "Bruh my keyboard isn't working", 'Omg bruh I chose the wrong form'];
                    sockets.broadcast(messages[Math.floor(Math.random() * messages.length)]);
...
class io_nearestDifferentMaster2 extends ioTypes.nearestDifferentMaster {
    ...
    think(input) {
        ...
        if (
            this.targetLock &&
            (!this.validate(this.targetLock, this.body, this.body.master.master, range * range, (range * range * 4) / 3) || this.wouldHitWall(this.body, this.targetLock)) // Very expensive
        ) {
            this.targetLock = undefined;
            this.tick = 100;
        }
        ...
        if (this.firingAtMe && damageRef.collisionArray.length && damageRef.health.display() < this.oldHealth) {
            ...
            if (this.validTargets.indexOf(damageRef.collisionArray[0]) === -1) {
                let a = damageRef.collisionArray[0].master.id === -1 ? damageRef.collisionArray[0].source : damageRef.collisionArray[0].master;
                if (this.body.firingArc == null || this.body.aiSettings.view360 || Math.abs(util.angleDifference(util.getDirection(this.body, a), this.body.firingArc[0])) < this.body.firingArc[1]) {
                    this.targetLock = a;
...
Class.toothlessBossTurret = {
    ...
    CONTROLLERS: ['onlyAcceptInArc', ['nearestDifferentMaster2', {lookAtDanger: false, firingAtMe: true, timeout: 10}]],
    COLOR: 'grey',
    GUNS: [
        {
    ...
    ON: [
        {
            ...
            handler: ({body}) => {
                const master = body.master;
                if (master._maxPower) body.color.base = colorArray[Math.floor(master._power / (master._maxPower / arraySize)) > arraySize - 1 ? arraySize - 1 : Math.floor(master._power / (master._maxPower / arraySize))];
            },
        },
    ],
};
...
Class.toothlessBoss.TURRETS = Class.toothlessBoss.TURRETS.concat(
    weaponArray(
        [
            {
