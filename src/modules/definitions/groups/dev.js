...
function connectMatrix(matrix, matrix2Entrance) {
    ...
    for (let x = 0; x < matrixWidth; x++)
        for (let y = 0; y < matrixHeight; y++) {
            ...
            matrix[y][x].UPGRADES_TIER_0 = ['developer', top, 'spectator', left, center, right, 'basic', bottom, matrix2Entrance];
        }
}
let generatorMatrix = [
    ...
    for (let y = 0; y < tensorHeight; y++) {
        for (let z = 0; z < tensorLength; z++) {
            for (let w = 0; w < tensorDepth; w++) {
                ...
                Class[labyTensor[x][y][z][w]].UPGRADES_TIER_0 = ['developer', left, right, 'teams', top, bottom, 'eggGenerator', front, back, 'PowerGemGenerator', past, future];
            }
        }
    }
}
