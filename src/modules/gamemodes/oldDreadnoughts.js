                ...
                types: [
                    ...
                    },
                    {
                        ...
                        handler: (entity) => {
                            ...
                            entity.define({
                                // Purge all unwanted entity config
                                STAT_NAMES: {},
                                IS_SMASHER: false,
                                ALPHA: [0, 1],
                            });
        ...
        for (let batch of this.spawnBatches) {
            for (let portal of batch.types) {
                ...
                entity.on('collide', ({instance, other}) => {
                    ...
                    // Validity checking
                    if (other.type != 'tank') {
                        if (other.type != 'miniboss' && other.type != 'food' && other.type != 'aura' && other.type != 'wall' && other.type != 'unknown' && (other.x - entity.x) ** 2 + (other.y - entity.y) ** 2 <= 625) {
                            other.kill();
                        }
                        return;
                    }
