...
class Gun extends EventEmitter {
    ...
    checkShootPermission() {
        let shootPermission = this.maxChildren ? this.maxChildren > this.children.length * this.childrenLimitFactor : this.body.maxChildren ? this.body.maxChildren > this.body.children.length * this.childrenLimitFactor : true;
    ...
    createBullet(spawnX, spawnY) {
        ...
        if (this.body.velocity.length) {
            let extraBoost = Math.max(0, velocity.x * this.body.velocity.x + velocity.y * this.body.velocity.y) / this.body.velocity.length / velocity.length;
            if (extraBoost) {
        ...
        // Spawn bullet
        (spawnX = this.body.x + this.body.size * spawnX - velocity.x), (spawnY = this.body.y + this.body.size * spawnY - velocity.y);
    ...
    calculateBulletStats() {
        ...
        for (let property in out) {
            if (this.bulletBodyStats[property] == null) continue;
...
class Entity extends EventEmitter {
    ...
    giveUp(player, name = 'Mothership') {
        if (!player.body.isMothership) {
            player.body.controllers = [new ioTypes.nearestDifferentMaster(player.body), new ioTypes.spin(player.body, {onlyWhenIdle: true})];
        } else {
            player.body.controllers = [new ioTypes.nearestDifferentMaster(player.body), new ioTypes.wanderAroundMap(player.body, {lookAtGoal: true}), new ioTypes.mapTargetToGoal(player.body)];
        }
            ...
            case 'swarm': {
                ...
                if (gactive && l > this.size) {
                    let XvelDesired = (this.topSpeed * g.x) / l,
                        ...
                        turning = Math.sqrt((this.topSpeed * Math.max(1, this.range) + 1) / this.scaledAcceleration);
                    engine = {
    ...
    friction() {
        ...
        if (excess > 0 && this.damp) {
            ...
            if (Config.SPACE_PHYSICS) finalvelocity *= this.type === 'bullet' ? 1.005 : 1.1;
    ...
    contemplationOfMortality() {
        ...
        if (this.isDead()) {
            ...
            let name = this.master.name == '' ? (this.master.type === 'tank' ? 'an unnamed ' + this.label : this.master.type === 'miniboss' ? 'a visiting ' + this.label : this.label.substring(0, 3) == 'The' ? this.label : util.addArticle(this.label)) : this.master.name + "'s " + this.label;
