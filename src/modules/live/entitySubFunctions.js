...
class HealthType {
    ...
    getDamage(amount, capped = true) {
        ...
        switch (this.type) {
            case 'dynamic':
                return Math.max(capped ? Math.min(amount * this.permeability, this.amount) : amount * this.permeability, damageToMax);
            case 'static':
                return Math.max(capped ? Math.min(amount, this.amount) : amount, damageToMax);
        }
    }
