...
function firmcollide(my, n, buffer = 0) {
    ...
    let cycles = 0;
    while (dist <= my.realSize + n.realSize && !(strike1 && strike2) && cycles < 150) {
        cycles += 1;
        strike1 = false;
        strike2 = false;
...
function advancedcollide(my, n, doDamage, doInelastic, nIsFirmCollide = false) {
    let tock = Math.min(my.stepRemaining, n.stepRemaining),
        ...
        delta = new Vector(tock * (motion._me.x - motion._n.x), tock * (motion._me.y - motion._n.y)),
    ...
    let reductionFactor = 1,
        ...
        accelerationFactor = delta.length ? combinedRadius / 4 / (Math.floor(combinedRadius / delta.length) + 1) : 0.001,
        depth = {
    ...
    if (doDamage) {
        ...
        if (!bail) {
            ...
            my.damageReceived += __my * Number(__my > 0 ? my.team != n.team : n.healer && n.team == my.team && my.type == 'tank' && n.master.id != my.id);
            n.damageReceived += __n * Number(__n > 0 ? my.team != n.team : my.healer && n.team == my.team && n.type == 'tank' && my.master.id != n.id);
        }
    }
    ...
    if (nIsFirmCollide < 0) {
        ...
        let spring = (2 * Math.sqrt(savedHealthRatio._me * savedHealthRatio._n)) / Config.runSpeed,
            ...
            springImpulse = Config.KNOCKBACK_CONSTANT * spring * combinedDepth.up,
...
function mazewallcollide(wall, bounce) {
    ...
    if (bounce.x + bounce.size < wall.x - trueWallSize || bounce.x - bounce.size > wall.x + trueWallSize || bounce.y + bounce.size < wall.y - trueWallSize || bounce.y - bounce.size > wall.y + trueWallSize) return 0;
