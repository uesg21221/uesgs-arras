...
Object.defineProperties(room, {
    tileWidth: {
        get: () => Config.TILE_WIDTH,
        set: (v) => {
            Config.TILE_WIDTH = v;
        },
    },
    tileHeight: {
        get: () => Config.TILE_HEIGHT,
        set: (v) => {
            Config.TILE_HEIGHT = v;
        },
    },
    width: {
        get: () => room.xgrid * Config.TILE_WIDTH,
        set: (v) => {
            Config.TILE_WIDTH = v / room.xgrid;
        },
    },
    height: {
        get: () => room.ygrid * Config.TILE_HEIGHT,
        set: (v) => {
            Config.TILE_HEIGHT = v / room.ygrid;
        },
    },
});
...
Object.defineProperties(room.center, {
    x: {
        get: () => (room.xgrid * Config.TILE_WIDTH) / 2,
        set: (v) => {
            Config.TILE_WIDTH = (v * 2) / room.xgrid;
        },
    },
    y: {
        get: () => (room.ygrid * Config.TILE_HEIGHT) / 2,
        set: (v) => {
            Config.TILE_HEIGHT = (v * 2) / room.ygrid;
        },
    },
});
...
room.random = function () {
    return Config.ARENA_TYPE === 'circle'
        ? room.near(room.center, room.center.x)
        : {
...
class TileEntity {
    constructor(tile, loc) {
        if (!(tile instanceof Tile)) {
            throw new Error(`The cell at ${loc.x},${loc.y} in the room setup is not a Tile object!` + ('string' == typeof tile ? ' But it is a string, which means you probably need to update your room setup!' : ' But it is of type ' + typeof tile));
        }
        let gridLoc = (this.gridLoc = {x: parseFloat(loc.x), y: parseFloat(loc.y)});
        // this.blueprint = tile.args;
        this.loc = {
            get x() {
                return room.tileWidth * (gridLoc.x + 0.5);
            },
            get y() {
                return room.tileHeight * (gridLoc.y + 0.5);
            },
        };
